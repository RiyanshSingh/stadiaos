...
export function FanCopilot() {
  ...
  const handleIncidentSubmit = async (data: any) => {
    ...
    try {
      await reportIncident(
        { type: data.incidentType, zone: data.locationLabel, description: data.description },
        match.id,
        match.stadium_id,
        profile.id,
        profile.role
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0a0a0a] text-white overflow-hidden font-sans">
      ...
      {/* HEADER & CONTEXT BAR */}
      <div className="pt-12 px-5 pb-4 bg-black/80 backdrop-blur-2xl z-30 border-b border-white/5">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="w-10 h-10 flex-shrink-0 bg-white/5 hover:bg-white/10 rounded-full flex items-center justify-center transition-colors border border-white/10">
            <ChevronLeft className="w-5 h-5 text-white/70" />
          </button>
          
          <div className="flex-1 overflow-x-auto hide-scrollbar">
            <div className="flex gap-2 w-max">
              <div className="flex items-center h-8 gap-2 px-3 rounded-md bg-white/10 text-[10px] font-bold tracking-widest uppercase text-white shadow-sm border border-white/10">
                <span className="w-1.5 h-1.5 rounded-full bg-white/90 animate-pulse" /> Match: {activeMatchStatus}
              </div>
              <div className="flex items-center h-8 gap-2 px-3 rounded-md bg-white/5 text-[10px] font-bold tracking-widest uppercase text-white/70 border border-white/5">
        ...
        {!hasStarted ? (
          <div className="h-full flex flex-col justify-end pb-4">
            <motion.div 
              ...
              className="mb-8"
            >
              <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center mb-5 border border-white/10 shadow-lg">
                <Sparkles className="w-6 h-6 text-white" />
              </div>
              <h2 className="text-2xl font-bold tracking-tight text-white mb-2">How can I help?</h2>
              <p className="text-[15px] font-medium text-white/50 leading-relaxed max-w-[90%]">
                I'm your StadiaOS Copilot. I can find facilities, guide you to your seat, or assist with incidents.
              </p>
            </motion.div>
            
            <div className="space-y-3">
              <p className="text-[10px] uppercase font-bold tracking-widest text-white/30 ml-1">Try asking</p>
...
fs.writeFileSync('src/features/fan-assistant/FanCopilot.tsx', content);
