...
import { SECTION_GATE_MAP } from '@/lib/constants/stadiumData'
...
export function ProfileView() {
  ...
  const activeIncident = incidents.find(i => i.reporterId === profile?.id) || null;
            ...
            <div className="mb-6">
              ...
              {ticket ? (
                <div className="flex gap-1.5 overflow-x-auto hide-scrollbar">
                  <div className="px-2 py-1 rounded-md border border-white/10 bg-white/5 flex items-baseline whitespace-nowrap">
                    <span className="text-[10px] text-white/50 uppercase tracking-widest font-bold mr-1">Sec</span>
                    <span className="text-base font-bold tracking-tight text-white">{ticket.seat_section}</span>
                  </div>
                  <div className="px-2 py-1 rounded-md border border-white/10 bg-white/5 flex items-baseline whitespace-nowrap">
                    <span className="text-[10px] text-white/50 uppercase tracking-widest font-bold mr-1">Row</span>
                    <span className="text-base font-bold tracking-tight text-white">{ticket.seat_row || '-'}</span>
                  </div>
                  <div className="px-2 py-1 rounded-md border border-white/10 bg-white/5 flex items-baseline whitespace-nowrap">
                    <span className="text-[10px] text-white/50 uppercase tracking-widest font-bold mr-1">Seat</span>
                    <span className="text-base font-bold tracking-tight text-white">{ticket.seat_number || '-'}</span>
                  </div>
                  <div className="px-2 py-1 rounded-md border border-white/10 bg-white/5 flex items-baseline whitespace-nowrap ml-auto">
                    <span className="text-[10px] text-white/50 uppercase tracking-widest font-bold mr-1">Gate</span>
                    <span className="text-base font-bold tracking-tight text-white">{computedGate?.replace(/Gate\s+/i, '') || '-'}</span>
                  </div>
                </div>
              ) : (
                <p className="text-lg font-bold tracking-tight text-white/50 mt-1">No Ticket</p>
              )}
            </div>
            
            <div className="flex gap-3">
              <Link to="/route/seat" className="flex-1">
              ...
              <Link to="/route/exit" className="flex-1">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-2 mb-1">
                    <h3 className="font-semibold tracking-tight text-[15px] truncate">{activeIncident ? `${activeIncident.type} at ${activeIncident.zone}` : 'No active reports'}</h3>
                    <span className="text-[10px] uppercase font-bold tracking-widest px-2 py-1 bg-white/5 rounded-full text-white/60 shrink-0">{activeIncident ? activeIncident.status : '---'}</span>
                  </div>
                  <p className="text-[13px] text-white/50 leading-relaxed font-medium">{activeIncident ? activeIncident.description : 'You have not reported any recent incidents.'}</p>
                </div>
              </div>
            </Card>
          </Link>
