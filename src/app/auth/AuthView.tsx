...
export function AuthView() {
  ...
  useEffect(() => {
    if (userId && role) {
      // Determine where they intended to go, or default
      const from = (location.state as any)?.from?.pathname;
      if (role === 'ops_manager') {
        navigate(from && from.startsWith('/ops') ? from : '/ops', { replace: true });
      } else if (role === 'fan') {
        // We will route them to '/' and let the protected route handle ticket-setup
        ...
        >
          <div className="mb-10 text-center flex flex-col items-center">
            ...
            <p className="text-[13px] text-white/50 uppercase tracking-widest font-bold">
              {isSignUp ? 'Create Fan Account' : 'Identity Gateway'}
            </p>
          </div>
          ...
          <Card className="glass-card p-6 rounded-[2rem] border border-white/10 bg-white/[0.02] shadow-2xl">
            ...
            <div className="mt-6 text-center">
              <button
                ...
                className="text-[13px] text-white/40 hover:text-white/80 font-medium transition-colors"
              >
          ...
          <div className="mt-8 text-center text-[11px] text-white/30 uppercase tracking-widest font-bold">
