...
export function OperationsPanel() {
  ...
  return (
    <div className="space-y-10">
      {/* Gates */}
      <section>
        <h2 className="text-[11px] font-bold text-white/70 uppercase tracking-widest mb-4 px-2">Gate Operations</h2>
        {gates.length === 0 ? (
          <div className="glass-card p-8 rounded-3xl border border-white/5 bg-white/[0.02] text-center">
             ...
             <p className="text-[11px] font-bold uppercase tracking-widest text-white/70">No Gate Data</p>
          </div>
        ) : (
              ...
              {gates.map(gate => (
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 text-[11px] font-bold text-white/70 uppercase tracking-widest">
                      <span>Wait:</span>
                  ...
                  <div className={cn(
                    ...
                    gate.crowd === 'Low' ? "bg-white/5 text-white/70 border-transparent" :
      ...
      {/* Facilities */}
      <section>
        <h2 className="text-[11px] font-bold text-white/70 uppercase tracking-widest mb-4 px-2">Facility Queues</h2>
        {facilities.length === 0 ? (
           <div className="glass-card p-8 rounded-3xl border border-white/5 bg-white/[0.02] text-center">
             <AlertCircle className="w-6 h-6 text-white/20 mx-auto mb-3" />
             <p className="text-[11px] font-bold uppercase tracking-widest text-white/70">No Facility Data</p>
          </div>
        ) : (
          <Card className="glass-card p-0 rounded-3xl border border-white/5 bg-white/[0.02] overflow-hidden">
            <div className="divide-y divide-white/[0.05]">
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="pr-4">
                      ...
                      <p className="text-[11px] font-bold text-white/70 uppercase tracking-widest">{facility.type} • {facility.zone}</p>
                    </div>
                    <div className={cn(
                      "shrink-0 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest border",
                      facility.crowd === 'Low' ? "bg-white/5 text-white/70 border-transparent" :
                  ...
                  <div className="flex items-center justify-between mt-1 pt-3 border-t border-white/5">
                    <span className="text-[10px] uppercase font-bold tracking-widest text-white/70">Queue Est.</span>
